    ...
    public function handleFrontRoutes($slug)
    {
        ...
        switch ($slug->reference_type) {
            ...
            case Category::class:
                ...
                $newsSlug = $slugHelper0->getSlug('news', '', "Botble\Page\Models\Page");
                $condition = [
                    'id' => $newsSlug->reference_id,
                    'status' => BaseStatusEnum::PUBLISHED,
                ];
                $pageForNews = app(PageInterface::class)->getFirstBy($condition, ['*'], ['slugable']);

                Theme::breadcrumb()
                    ->add(__('Home'), url('/'))
                    ->add($pageForNews->name, url('/news'))
