...
class SlugHelper
{
    ...
    public function getPagePrefixList(string $model, array $default = ['']) // This function is just for suburl pages
    {
        $condition = ['reference_type' => $model];
        $arrResult = app(SlugInterface::class)->allBy($condition, [], ["prefix"])->unique("prefix")->pluck("prefix");
        return $arrResult;
    }
    ...
    public function getPagePrefix(string $model, int $slug_id)
    {
        $condition = ['id' => $slug_id];
        return app(SlugInterface::class)->getFirstBy($condition, ["prefix"], [])->prefix;
    }
