...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function addSlugBox($object = null)
    {
        if ($object && SlugHelper::isSupportedModel(get_class($object))) {
            ...
            $modifiedSlug = $object->slug;

            if (get_class($object) == "Botble\Page\Models\Page" && $object->slug_id != "")
            {
                $pagePrefix = SlugHelper::getPagePrefix(Page::class, $object->slug_id);
                $modifiedSlug = $pagePrefix ? $pagePrefix . "/" . $object->slug : $object->slug;
            }

            return view('packages/slug::partials.slug', compact('object', 'prefix', 'modifiedSlug'))->render();
        }
